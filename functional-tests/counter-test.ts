...
const initializeContract = async (client: SecretNetworkClient, contractPath: string) => {
...
async function fillUpFromFaucet(client: SecretNetworkClient, targetBalance: Number) {
  ...
  console.log(`Got tokens from faucet: ${balance} SCRT for ${client.address}`)
}
...
async function queryCount(client: SecretNetworkClient, contractHash: string, contractAddress: string): Promise<number> {
...
async function resetTx(client: SecretNetworkClient, contractHash: string, contractAddess: string) {
  const tx = await client.tx.compute.executeContract(
    {
      sender: client.address,
      contractAddress: contractAddess,
...
async function test_count_on_intialization(client: SecretNetworkClient, contractHash: string, contractAddress: string) {
  const onInitializationCounter: number = await queryCount(client, contractHash, contractAddress)
  assert(
...
async function test_increment_stress(client: SecretNetworkClient, contractHash: string, contractAddress: string) {
  const onStartCounter: number = await queryCount(client, contractHash, contractAddress)
  ...
  const afterStressCounter: number = await queryCount(client, contractHash, contractAddress)
  assert(
...
async function runTestFunction(
  tester: (client: SecretNetworkClient, contractHash: string, contractAddress: string) => void,
...
;(async () => {
  ...
  await runTestFunction(test_count_on_intialization, client, contractHash, contractAddress)
  await runTestFunction(test_increment_stress, client, contractHash, contractAddress)
